...
if not os.path.exists("./breastModel.pth"):
    num_classes = (
    ...
    plt.legend()
    save_path = os.path.join("./breastimages1", "loss curve")
    ...
    model_path = "./breastModel.pth"  # Replace with your saved model path
...
# Set up a figure to hold the subplots for the 10 samples
fig, axes = plt.subplots(5, 3, figsize=(10, 14))  # 10 rows and 3 columns
...
for test_image in test_image_paths:
    ...
    if showcount < 5:
        axes[showcount, 0].imshow(image)
        axes[showcount, 0].set_title("Original Image")
        axes[showcount, 0].axis("off")

        axes[showcount, 1].imshow(label)
        axes[showcount, 1].set_title("Ground Truth Mask")
        axes[showcount, 1].axis("off")

        axes[showcount, 2].imshow(predict_label_resize, cmap="gray")
        axes[showcount, 2].set_title("Predicted Mask")
        axes[showcount, 2].axis("off")
    showcount += 1
...
plt.tight_layout()
combined_image_save_path = os.path.join("./breastimages1", "combined_predictions.jpg")
plt.savefig(combined_image_save_path)
